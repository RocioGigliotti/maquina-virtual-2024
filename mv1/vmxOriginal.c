...
int main(int argc, char const *argv[]){
    ...
    if (argc == 1) 
        printf("Se debe agregar la ruta de un archivo para ejecutar\n");
     else if (argc == 2) {
        nombreArch = argv[1];
...
void disassembly(TMV mv){
    //printf("\nDISASSEMBLEando... beep boop beep\n");
    ...
    while(mv.registros[IP].valor < mv.tabla_segmentos[CS].size){
        printf("[%04X] ", mv.registros[IP].valor & 0x000000FF);
...
int getop(operando opA, TMV mv){
    ...
    switch (opA.tipo){
        case 0: {// memoria
            ...
            int dirFisica = mv.tabla_segmentos[(mv.registros[codReg].valor>> 16) & 0x00000F].base + offset;
        ...
        case 2: {//registro
            ...
            switch (sectorReg){
                ...
                case 1: // 4to byte del registro
                    valor  = mv.registros[codReg].valor & 0x000F;
                    break;

                case 2: // 3er byte del registro
                    valor  = mv.registros[codReg].valor & 0x00F0;
                    break;

                case 3: //registro de 2 bytes
                    valor  = mv.registros[codReg].valor & 0x00FF;
                    break;
            }
            break;
        }   
...
void SYS1(operando opA, operando opB, TMV* mv){ //READ
    ...
    for (int i=0; i<cantCeldas; i++){
        printf("Leyendo la celda %d / %d\n", i, cantCeldas);
        printf("[%04d]: ", dirFisica);
        switch (modoLectura){
            case 0x08: 
                scanf("%X", &dato);
                break;
        ...
        for (int j=tamanioCeldas; j > 0; j--){
            printf ("j = %d, tamanioCeldas= %d\n", j, tamanioCeldas);
            mv -> memoria[k + j - 1] = dato & 0x000000FF;
            dato = dato >> 8;         
        }
        dirFisica += tamanioCeldas;
    }
    printf("\nel dato en memoria[33] es %d", mv->memoria[33]);
    printf("\nel dato en memoria[34] es %d", mv->memoria[34]);
    printf("\nel dato en memoria[35] es %d", mv->memoria[35]);
    printf("\nel dato en memoria[36] es %d", mv->memoria[36]);
    scanf("%d", &dato);
}
...
void SYS2(operando opA, operando opB, TMV* mv){ // WRITE
    ...
    for (int i=0; i<cantCeldas; i++){
        ...
        if (modoEscritura & 0x00000002 != 0){ //caracter
            i=0;
            while (i<tamanioCeldas){
                aux = (dato >> (8*(tamanioCeldas-1-i++))) & 0x000000FF;
                if ((aux<33) | (aux>126))
                    printf(". ");
                else
                    printf("%c", aux );
        ...
        if (modoEscritura & 0x00000008){ //hexadecimal
            printf(" %se%");
            i=0;
            while (i<tamanioCeldas){
                printf("%X", ((dato>>(8*(tamanioCeldas-1-i++))) & 0x000000FF));
            }
        }
        printf("\n");            
    }
